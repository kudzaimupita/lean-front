    ...
    <>
      {imgList.map((img) => (
        <div className="group relative rounded border p-2 flex" key={img}>
          <img
            className="rounded max-h-[100px] max-w-full"
...
const ProductImages = (props) => {
  ...
  const docs = values.documents.map((doc) => {
    console.log(`https://baboon-images.s3.amazonaws.com/${doc}`);
    return {
      uri: `https://baboon-images.s3.amazonaws.com/${doc}`,
    };
  });
  // console.log(docs);
  return (
