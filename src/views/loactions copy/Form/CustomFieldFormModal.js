...
import PaginatedSelectTable from "../../PaginatedSelectTable/Table";
import { getNewVehicles } from "../../../services/vehicleService";
import { useDispatch, useSelector } from "react-redux";
import { getData, setTableData } from "../../TableView/store/dataSlice";
import { useNavigate } from "react-router-dom";
import * as Yup from "yup";
import reducer from "../../PaginatedSelectTable/store/index";
...
injectReducer("paginatedSelect", reducer);
...
storeCustomFields({category:id},AssetTypeId.id)
setIsOpen(false)
  }
  const columns = useMemo(() => [
    {
      ...
      Cell: (props) => {
        ...
        return <a href='#' onClick={()=>handleSelect(row._id)}> <span className="capitalize">{row.name}</span></a>
      },
    },
  ]);
  return (
