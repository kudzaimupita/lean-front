...
import { getNewDataEntries } from "../../../services/dataEntry.service";
import ProductForm from "views/vehicles/EditAssetForm";
...
const ProductEdit = () => {
  ...
  useEffect(() => {
    ...
    getVehicle(path).then((data) => {
      console.log(data);
      const obj={}
      getNewDataEntries({ vehicle: data?._id }).then((fields) => {
    
        fields.data.results?.map((dataEntry)=>{
          console.log(dataEntry)
       obj[`customField-${dataEntry.customField._id}`]=dataEntry.value
         })
        console.log(fields.data.results);
        data.newFields = fields.data.results;
        setVehicle({...data,...obj});
