      ...
      <MenuCollapse
        label={
          <>
            ...
            <span className="text-gray-400 hover:bg-sky-700">
        ...
        {nav.subMenu.map((subNav) => (
          ...
          >
            <MenuItem eventKey={subNav.key}>
              {subNav.path ? (
                <Link
                  className="h-full w-full flex items-center text-gray-400 "
                  onClick={() =>
                    onLinkClick?.({
                      key: subNav.key,
                      title: subNav.title,
...
const CollapsedItem = ({ nav, onLinkClick, userAuthority, direction }) => {
  const menuItem = (
    <MenuItem key={nav.key} eventKey={nav.key} className="mb-2 text-gray-400 ">
      <VerticalMenuIcon icon={nav.icon} />
    </MenuItem>
  );
